...
func main() {
	...
	router.HandleFunc("/api/v1/cars", createCar).Methods(http.MethodPost)            // add a new car
	router.HandleFunc("/api/v1/cars/{id}", updateCarById).Methods(http.MethodPut)    // update a car
	router.HandleFunc("/api/v1/cars/{id}", deleteCarById).Methods(http.MethodDelete) // delete a car
